...
// import { storage } from "./storage";
// import { authMiddleware, requireAuth } from "./services/auth";
// import { loginSchema, signupSchema } from "../../shared/schema"; // Adjusted path
// import { processMessage } from "./services/gemini";
// import { parsePDF } from "./services/pdf-parser";
// import { uploadToSupabase, deleteFromSupabase } from "./services/supabase";
// import {
...
// export async function registerRoutes(app: Express): Promise<Server> {
//   // All other routes (auth, file upload, etc.) remain the same...
//   app.use(authMiddleware);
...
//   // THE FIX: Updated Chat Route Logic
//   app.post("/api/chat", requireAuth, async (req, res) => {
//     try {
//       const { message, fileId, conversationId } = req.body;
...
//         // Fetch PDF content from permanent storage, not the temporary 'uploads' folder
//         if (config.NODE_ENV === "development" || !config.DATABASE_URL) {
//           // In development, read from the local storage path
//           const localPath = getLocalFilePath(file.filePath);
//           pdfContent = await parsePDF(localPath);
//         } else {
//           // In production, download from Supabase to a temporary path to parse
//           // This requires a new helper function in your supabase service
//           const tempPath = await storage.downloadFromSupabase(file.filePath);
//           pdfContent = await parsePDF(tempPath);
//           fs.unlinkSync(tempPath); // Clean up the temporary downloaded file
//         }
...
//         return res
//           .status(400)
//           .json({ message: "Either conversationId or fileId is required" });
//       }
...
//           pdfContent = await parsePDF(localPath);
//         } else {
//           // Assuming a download helper exists for production
//           const tempPath = await storage.downloadFromSupabase(file.filePath);
//           pdfContent = await parsePDF(tempPath);
//           fs.unlinkSync(tempPath);
//         }
//       } else {
//         // This prevents creating a conversation without a file.
//         return res.status(400).json({ message: "A file ID is required to start a new conversation." });
//       }
...
//   app.get("/api/conversations", requireAuth, async (req, res) => {
//     try {
//       // This new endpoint gets conversations with their linked file names
//       const conversations = await storage.getConversationsWithFiles(req.session.userId!);
//       res.json(conversations);
//     } catch (error) {
//       console.error("Get conversations error:", error);
//       res.status(500).json({ message: "Failed to get conversations" });
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/conversations", requireAuth, async (req, res) => {
    try {
      ...
      // --- THIS IS THE FINAL TIMESTAMP FIX ---
      // We ensure every `createdAt` is a proper ISO string before sending.
